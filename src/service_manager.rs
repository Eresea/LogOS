...
impl ProgramManager {
    ...
    pub fn request(&mut self, request: ManagerRequest, rights: ManagerRights) -> ManagerDecision {
        ...
        match request.operation {
            ...
            ManagerOperation::ProgramStatus => {
                let Some(index) = self.valid_program_index(&request) else {
                    response.status = ManagerStatus::NotFound;
                    return ManagerDecision { response, action: ManagerAction::None };
                };
    ...
    fn valid_program_index(&self, request: &ManagerRequest) -> Option<usize> {
        if request.program_slot == u8::MAX {
            return self.find_program(request.name());
        }
        ...
        (program.state != ManagerState::Vacant
            ...
            && program.generation == request.program_generation)
            .then_some(index)
    }
}
...
impl ServiceManager {
    pub const fn new() -> Self {
        let mut manager = Self {
            ...
            programs: [ProgramSlot::EMPTY; MAX_PROGRAM_SLOTS],
        };
...
mod tests {
    ...
    #[test]
    fn program_handles_are_generation_safe_and_name_bound() {
        let mut manager = manager();
        let request = ManagerRequest::new(ManagerOperation::ProgramStart, 7)
