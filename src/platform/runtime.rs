...
#[cfg(feature = "test-hooks")]
#[path = "runtime_network_test.rs"]
mod runtime_network_test;
#[cfg(feature = "test-hooks")]
use runtime_network_test::{network_status_label, run_network_request, run_network_tcp_stream};
...
pub(crate) fn run(
    ...
    macro_rules! poll_terminal_once {
        () => {{
            let mut ready = false;
            for _ in 0..256 {
                if native_scheduler
                ...
                if matches!(
                    remote_load_phase,
                    RemoteLoadPhase::Enrollment | RemoteLoadPhase::Control
                ) && !poll_remote_load_once!()
                {
                    break;
                }
    ...
    test_hooks::serve(
        ...
        |action| match action {
            test_hooks::Action::Input(value) => {
                if !poll_terminal_once!() || !poll_remote_load_once!() {
                    return false;
                }
