...
/// Offline Inter ASCII coverage atlases (ADR-0088), generated by
/// `scripts/gen_inter_atlas.py` from `third_party/Inter-Regular.ttf`. Advance
/// widths (the measurement API layout uses) live in `logos_abi` so the UI
/// scene builder can size and center labels without depending on Display;
/// only the raster coverage bytes live here, since Display remains the sole
/// framebuffer writer. Proportional, ASCII-only, no kerning or shaping.
const INTER_BODY_DATA: &[u8] = include_bytes!("inter_body_16x18.bin");
const INTER_TITLE_DATA: &[u8] = include_bytes!("inter_title_22x25.bin");

const _: () = assert!(
    INTER_BODY_DATA.len()
        == (logos_abi::INTER_GLYPH_COUNT + 1)
            * logos_abi::INTER_BODY_CELL.0
            * logos_abi::INTER_BODY_CELL.1
);
const _: () = assert!(
    INTER_TITLE_DATA.len()
        == (logos_abi::INTER_GLYPH_COUNT + 1)
            * logos_abi::INTER_TITLE_CELL.0
            * logos_abi::INTER_TITLE_CELL.1
);

/// Which glyph run a `GlyphRun` command selects. Terminal and the retained
/// `TextGrid` node never carry the Inter font flags, so they always resolve
/// to `Mono` and keep addressing the fixed 8x16 JetBrains Mono atlas.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TextFont {
    Mono,
    Inter(logos_abi::InterStyle),
}

impl TextFont {
    pub const fn from_flags(flags: u32) -> Self {
        if flags & logos_abi::GUI_TEXT_FLAG_FONT_INTER_TITLE != 0 {
            TextFont::Inter(logos_abi::InterStyle::Title)
        } else if flags & logos_abi::GUI_TEXT_FLAG_FONT_INTER_BODY != 0 {
            TextFont::Inter(logos_abi::InterStyle::Body)
        } else {
            TextFont::Mono
        }
    }

    /// Fixed raster cell (width, height) in pixels.
    pub const fn cell(self) -> (usize, usize) {
        match self {
            TextFont::Mono => (GLYPH_WIDTH, GLYPH_HEIGHT),
            TextFont::Inter(style) => style.cell(),
        }
    }
}

fn inter_atlas(style: logos_abi::InterStyle) -> &'static [u8] {
    match style {
        logos_abi::InterStyle::Body => INTER_BODY_DATA,
        logos_abi::InterStyle::Title => INTER_TITLE_DATA,
    }
}

/// Coverage of one Inter glyph at local cell coordinates `(x, y)`. Unknown
/// scalars fall back to the replacement glyph, matching the JetBrains Mono
/// path. Direct atlas indexing needs no cache: ASCII glyphs carry no
/// runtime-computed overlays (unlike Mono's accented Latin-1 fallbacks).
pub fn inter_glyph_coverage(style: logos_abi::InterStyle, scalar: u32, x: usize, y: usize) -> u8 {
    let (width, height) = style.cell();
    if x >= width || y >= height {
        return 0;
    }
    let index = logos_abi::inter_glyph_index(normalize_scalar(scalar));
    let offset = index * width * height + y * width + x;
    inter_atlas(style)[offset]
}
...
mod tests {
    ...
    #[test]
    fn inter_atlases_assert_their_committed_size_at_compile_time() {
        // The `const _: () = assert!(...)` checks above already fail the
        // build if the atlas file sizes drift from ADR-0088's fixed cells;
        // this just documents the expectation the compile-time checks hold.
        let (body_w, body_h) = logos_abi::INTER_BODY_CELL;
        let (title_w, title_h) = logos_abi::INTER_TITLE_CELL;
        assert_eq!(INTER_BODY_DATA.len(), (logos_abi::INTER_GLYPH_COUNT + 1) * body_w * body_h);
        assert_eq!(INTER_TITLE_DATA.len(), (logos_abi::INTER_GLYPH_COUNT + 1) * title_w * title_h);
    }

    #[test]
    fn inter_unknown_scalar_renders_the_replacement_glyph() {
        let replacement = inter_glyph_coverage(
            logos_abi::InterStyle::Body,
            logos_abi::INTER_ASCII_LAST + 1,
            1,
            1,
        );
        let known = inter_glyph_coverage(logos_abi::InterStyle::Body, 0x1f600, 1, 1);
        assert_eq!(replacement, known);
    }

    #[test]
    fn inter_measurement_matches_raster_advance() {
        // The measurement API (`logos_abi::inter_text_width`) must agree with
        // what the raster path actually advances the cursor by, or labels
        // would be centered against a width Display never draws.
        for style in [logos_abi::InterStyle::Body, logos_abi::InterStyle::Title] {
            let text = b"Settings 123";
            let measured = logos_abi::inter_text_width(style, text);
            let summed: u32 = text
                .iter()
                .map(|&b| u32::from(logos_abi::inter_glyph_advance(style, u32::from(b))))
                .sum();
            assert_eq!(measured, summed);
            for &byte in text {
                let (cell_w, _) = style.cell();
                assert!(
                    (logos_abi::inter_glyph_advance(style, u32::from(byte)) as usize) <= cell_w
                );
            }
        }
    }
