...
pub const REMOTE_MESSAGE_PAYLOAD: usize = MAX_FRAME - 16 - REMOTE_MESSAGE_HEADER;
...
pub fn crypto_kat() -> Result<(), CryptoKatPhase> {
    ...
    const AEAD_PLAINTEXT: &[u8; 114] = b"Ladies and Gentlemen of the class of '99: If I could offer you only one tip for the future, sunscreen would be it.";
    const AEAD_CIPHERTEXT: [u8; 130] = [
    ...
    const MACHINE_PUBLIC: [u8; 32] = [
        0x5d, 0xfe, 0xdd, 0x3b, 0x6b, 0xd4, 0x7f, 0x6f, 0xa2, 0x8e, 0xe1, 0x5d, 0x96, 0x9d, 0x5b,
        0xb0, 0xea, 0x53, 0x77, 0x4d, 0x48, 0x8b, 0xda, 0xf9, 0xdf, 0x1c, 0x6e, 0x01, 0x24, 0xb3,
        0xef, 0x22,
    ];
    const CLIENT_PUBLIC: [u8; 32] = [
        0xa4, 0xe0, 0x92, 0x92, 0xb6, 0x51, 0xc2, 0x78, 0xb9, 0x77, 0x2c, 0x56, 0x9f, 0x5f, 0xa9,
        0xbb, 0x13, 0xd9, 0x06, 0xb4, 0x6a, 0xb6, 0x8c, 0x9d, 0xf9, 0xdc, 0x2b, 0x44, 0x09, 0xf8,
        0xa2, 0x09,
    ];
    const FIRST: [u8; 96] = [
        0xce, 0x8d, 0x3a, 0xd1, 0xcc, 0xb6, 0x33, 0xec, 0x7b, 0x70, 0xc1, 0x78, 0x14, 0xa5, 0xc7,
        0x6e, 0xcd, 0x02, 0x96, 0x85, 0x05, 0x0d, 0x34, 0x47, 0x45, 0xba, 0x05, 0x87, 0x0e, 0x58,
        0x7d, 0x59, 0xd5, 0xea, 0x48, 0xb8, 0x7c, 0x25, 0x93, 0x12, 0xe4, 0x12, 0x4f, 0x5b, 0x8a,
        0xb0, 0x9d, 0xba, 0xbf, 0xee, 0xac, 0x5b, 0x1a, 0x72, 0x2c, 0x5a, 0x59, 0xf2, 0x72, 0xaa,
        0xf5, 0x04, 0x60, 0x06, 0x33, 0x14, 0xd1, 0x67, 0x4c, 0x7e, 0xce, 0x1f, 0xfb, 0xa2, 0x87,
        0x65, 0x92, 0xba, 0x13, 0x4f, 0x71, 0x91, 0x82, 0x46, 0x9e, 0xa3, 0xd1, 0x84, 0x6d, 0x32,
        0x0e, 0x24, 0xce, 0x6f, 0xa5, 0x90,
    ];
    const SECOND: [u8; 48] = [
        0xac, 0x01, 0xb2, 0x20, 0x9e, 0x86, 0x35, 0x4f, 0xb8, 0x53, 0x23, 0x7b, 0x5d, 0xe0, 0xf4,
        0xfa, 0xb1, 0x3c, 0x7f, 0xcb, 0xf4, 0x33, 0xa6, 0x1c, 0x01, 0x93, 0x69, 0x61, 0x7f, 0xec,
        0xf1, 0x0b, 0x35, 0x83, 0xc2, 0x86, 0xe5, 0x76, 0x92, 0x30, 0xa1, 0x2f, 0xc6, 0xda, 0x99,
        0x5f, 0x08, 0xac,
    ];
    const TRANSPORT: [u8; 21] = [
        0x66, 0xec, 0x66, 0xfd, 0x7d, 0x27, 0x9d, 0x12, 0x70, 0x50, 0x6c, 0xd4, 0xa0, 0x41, 0x33,
        0x1b, 0xf8, 0x91, 0xa4, 0xb5, 0xb2,
    ];

    if sha2::Sha256::digest(b"abc").as_slice() != SHA256_ABC {
        return Err(CryptoKatPhase::Sha256);
    }
    if X25519::pubkey(&ALICE) != ALICE_PUBLIC {
        return Err(CryptoKatPhase::X25519Public);
    }
    if X25519::dh(&ALICE, &BOB_PUBLIC).ok() != Some(SHARED) {
        return Err(CryptoKatPhase::X25519Dh);
    }
    let cipher = ChaCha20Poly1305::new((&AEAD_KEY).into());
    let mut aead = [0; 130];
    aead[..AEAD_PLAINTEXT.len()].copy_from_slice(AEAD_PLAINTEXT);
    if cipher
        .encrypt_in_place_detached((&AEAD_NONCE).into(), &AEAD_AD, &mut aead[..114])
        .map(|tag| aead[114..].copy_from_slice(tag.as_slice()))
        .is_err()
        || aead != AEAD_CIPHERTEXT
    {
        return Err(CryptoKatPhase::ChaCha20Poly1305Encrypt);
    }
    let (ciphertext, tag) = aead.split_at_mut(114);
    if cipher
        .decrypt_in_place_detached((&AEAD_NONCE).into(), &AEAD_AD, ciphertext, tag.as_ref().into())
        .is_err()
        || ciphertext != AEAD_PLAINTEXT
    {
        return Err(CryptoKatPhase::ChaCha20Poly1305Decrypt);
    }

    use noise_protocol::{HandshakeStateBuilder, patterns::noise_ik};
    let machine = [3; 32];
    let client = [1; 32];
    if X25519::pubkey(&machine) != MACHINE_PUBLIC || X25519::pubkey(&client) != CLIENT_PUBLIC {
        return Err(CryptoKatPhase::NoiseFirstMessage);
    }
    ...
    initiator
        ...
        .set_s(client)
        .set_e([2; 32])
        .set_rs(MACHINE_PUBLIC);
    let mut initiator = initiator.build_handshake_state::<NoiseChaCha, NoiseSha256>();
    let mut first = [0; 96];
    initiator.write_message(&[], &mut first).map_err(|_| CryptoKatPhase::NoiseFirstMessage)?;
    if first != FIRST {
        return Err(CryptoKatPhase::NoiseFirstMessage);
    }
    ...
    let second_length = responder
        .accept_handshake(&first, &CLIENT_PUBLIC, &mut second)
        .map_err(|_| CryptoKatPhase::NoiseFirstMessage)?;
    if second_length != SECOND.len()
        || second[..second_length] != SECOND
        || initiator.read_message(&second[..second_length], &mut []).is_err()
        || !initiator.completed()
        || !responder.authorized()
    {
        return Err(CryptoKatPhase::NoiseSecondMessage);
    }
    let (mut send, _) = initiator.get_ciphers();
    let mut transport = [0; 21];
    send.encrypt(b"hello", &mut transport);
    if transport != TRANSPORT {
        return Err(CryptoKatPhase::Transport);
    }
    let mut plaintext = [0; 5];
    responder.open(&transport, &mut plaintext).map_err(|_| CryptoKatPhase::Transport)?;
    (plaintext == *b"hello" && responder.authorized())
        .then_some(())
        .ok_or(CryptoKatPhase::Transport)
}
...
mod tests {
    ...
    #[test]
    fn crypto_known_answers_and_noise_transcript() {
        assert_eq!(crypto_kat(), Ok(()));
    }
