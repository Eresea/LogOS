...
handle rejection, the typed in-process Runtime command/response lifecycle, the typed in-process
Health Ping command/response and restart/retry path, and a blocked task woken by another CPU for
SMP (same CPU for `-smp 1`). The runner captures debugcon output and fails on timeout or fatal
output.
