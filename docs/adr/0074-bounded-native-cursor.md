...
keeps cursor pixels out of the retained backbuffer. It paints a centered white
dot with a bounded black contour and soft halo directly over only the damaged
old/new cursor regions, restoring the underlying composition from the backbuffer
before each move. A left-button press adds a temporary subtle ring that is
removed on release.
