...
4. stage and validate a versioned native-service boot payload, add per-service address spaces, the native task loader, and the capability-only service contract, then run `logos-terminal` as a separately loaded service;
