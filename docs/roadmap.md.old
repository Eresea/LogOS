...
- [x] Define driver lifecycle: discover, bind, interrupt, quiesce, and recover without kernel-wide assumptions.
- [x] Add kernel panic/fault diagnostics, structured health reporting, and a defined recovery policy for failed drivers.
- [x] Expand the trace ring into fault and driver lifecycle diagnostics with an export path.
- [x] Add ACPI power-off and reset for a real `exit` path.
- [x] Expand QEMU integration checks to cover console input, IPC replies, blocked-task wake-up, and driver recovery.
