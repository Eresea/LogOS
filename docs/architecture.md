...
| Context boundary | `arch/context.rs` | timer, voluntary, ring-3 syscall, and user-fault entries save one GPR/RIP/RSP/RFLAGS/CS and x87/SSE frame shape |
...
| Process admission | `process::ProcessTable` | fixed 16-slot process model, ELF64 admission checks, one generation-safe address-space identity with 16 validated mappings per process, typed capability authorization, and exit/fault/reclaim outcomes |
