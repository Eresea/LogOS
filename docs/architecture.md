...
| Input service | `services/images/src/input` + `logos-input::InputDecoder` | consumes the Input-only PS/2 byte mapping, produces semantic key/text messages on the Input→Shell ring, and owns modifier/layout state |
| Shell service | `services/images/src/shell` + `logos-shell::Shell` | owns root/modal surface policy, focus, semantic input routing, bounded User claim/login/logout requests, credential clearing, restart invalidation, and typed session-context handoff to Flow |
| LockScreen service | `services/images/src/lockscreen` + `logos-lockscreen::LockScreen` | owns bounded username/password/confirmation editing, first-boot claim mode, retry/failure state, and password clearing; it receives no framebuffer or filesystem authority |
...
| Display service | `services/images/src/display` + `logos-display` | sole framebuffer writer; retains legacy cell diffs plus bounded root/modal GUI batches, validates generation-safe handles, coalesces damage, and rasterizes only event-driven invalidations into its mapped GOP framebuffer |
