...
| Session service | `services/images/src/session` + `logos-session::SessionService` | ring-3 owns bounded line editing and shell state, emits prompt/output in backpressured 256-byte chunks, and uses a compact in-memory volatile-file budget; no state survives restart or reboot |
