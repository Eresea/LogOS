...
| Storage boundary | Core VirtIO block adapter + `logos-storage` format + storage IPC adapter | Core owns PCI discovery, feature negotiation, fixed DMA arena, queues, interrupts, reset, timeouts, and flush; `logos-storage` owns fixed request lifecycles, superblocks, journal, replay, recovery, and durability; storage service requests use fixed ABI messages, process-bound capabilities, private staging pages, and kernel-owned IPC; paths and namespaces remain deferred |
