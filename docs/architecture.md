...
| LockScreen service | `services/images/src/lockscreen` + `logos-lockscreen::LockScreen` | owns bounded username/password editing, first-boot claim mode, retry/failure state, and password clearing; it receives no framebuffer or filesystem authority |
