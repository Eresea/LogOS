...
| Storage boundary | Core VirtIO block adapter + `logos-storage` format | Core owns PCI discovery, feature negotiation, fixed DMA arena, queues, interrupts, reset, timeouts, and flush; `logos-storage` owns fixed request lifecycles, superblocks, journal, replay, recovery, and durability; services use private staging pages and kernel-owned IPC; paths and namespaces remain deferred |
