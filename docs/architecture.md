...
| Control plane | `process` + `service_runtime` | admission-time typed capabilities and fixed service mappings; generic service syscalls are deferred |
...
| Terminal ABI | `logos-abi` | fixed semantic input, session stream, cell-diff render, endpoint identity, service identities, and capability kinds |
