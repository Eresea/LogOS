...
- Date: 2026-08-09
- Starting SHA: `79685e30c29687f930ed08ff57ac549cdb4d035b`
...
| Remote request | `src/platform/runtime.rs::poll_gateway` | Remote input is copied into one bounded generation-bound operation slot; current Invoke body still uses the compatibility persistence/session wrapper. | C/D | High | Finish multi-poll Storage/Sessions transitions before marking complete. |
...
The current Network stream architecture is compliant and remains the reference model. Sessions now
uses the same bounded runnable-notification boundary. Storage and Remote have operation slots and
