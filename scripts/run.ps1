...
& $qemuPath -machine q35 -m 256M -drive "if=pflash,format=raw,readonly=on,file=$ovmf" -drive "format=raw,file=fat:rw:$((Resolve-Path $esp).Path)" -device "virtio-balloon-pci,disable-modern=on,id=logos-virtio" -debugcon stdio -global isa-debugcon.iobase=0xe9
